...
Page({
  ...
  */
  goCat: function (isPay) {
    ...
    if (!this.data.productAttr.length) {
      that.addCartGoods(that.data.id, 1, productSelect, isPay)
      return false
    }
    ...
    that.addCartGoods(that.data.id, productSelect.cart_num, productSelect, isPay)
  },
  addCartGoods: function (productId, cart_num, productSelect, isPay) {
    var that = this
    postCartAdd({
      productId: productId,
      cartNum: cart_num,
      ...
      {console.log(res.data.cartId)
        wx.navigateTo({
          url: '/pages/coupons_mall/coupons_confirm/index?cartId=' + res.data.cartId
        });}
      else
