import { couponsConfirm} from '../../../api/couponsmall.js';
 ...
 */
  ...
  */
  getConfirm:function(){
    var that=this;
    couponsConfirm(this.data.cartId).then(res=>{
      console.log(res.data)
      that.setData({
  ...
  */
  ...
  SubOrder:function(e){
    var that = this, data={};
    // if (this.data.totalPrice > 0) return app.Tips({ title: '积分不足，无法兑换' });
    if (!this.data.btn_state) return app.Tips({ title: '积分不足，无法兑换' });
